...
$productsInCategoryQuery = "SELECT product_id, product_name, sku FROM products WHERE category_id = ? ORDER BY product_name";
$productsInCategoryStmt = $pdo->prepare($productsInCategoryQuery);
$productsInCategoryStmt->execute([$category_id]);
$productsInCategory = $productsInCategoryStmt->fetchAll();

// Fetch uncategorized products
$uncategorizedProductsQuery = "SELECT product_id, product_name, sku FROM products WHERE category_id IS NULL ORDER BY product_name";
$uncategorizedProductsStmt = $pdo->query($uncategorizedProductsQuery);
$uncategorizedProducts = $uncategorizedProductsStmt->fetchAll();
...
$categoriesStmt->execute([$category_id]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // Update category
        $updateCategoryStmt = $pdo->prepare("UPDATE categories SET category_name = ?, description = ? WHERE category_id = ?");
        $updateCategoryStmt->execute([$categoryName, $description, $category_id]);

        // Remove all products from this category
        $removeProductsStmt = $pdo->prepare("UPDATE products SET category_id = NULL WHERE category_id = ?");
        $removeProductsStmt->execute([$category_id]);

        // Add selected products to this category
        if (!empty($selectedItems)) {
            $placeholders = str_repeat('?,', count($selectedItems) - 1) . '?';
            $updateProductsStmt = $pdo->prepare("UPDATE products SET category_id = ? WHERE product_id IN ($placeholders)");
            $updateProductsStmt->execute(array_merge([$category_id], $selectedItems));
        }
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 1200px) {
            ...
            .other-categories, .edit-category {
                width: 100%;
            }
        }
...
<body>
    ...
    <div class="content">
        <div class="page-header">
            <h1 class="page-title">Edit Category: <?php echo htmlspecialchars($category['category_name']); ?></h1>
        </div>

        <div class="layout-container">
            <!-- Other Categories Section -->
            <div class="other-categories">
                <h2 class="section-header">Other Categories</h2>
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <li class="category-item">
                                ...
                                <div class="category-stats">
                                    <span><?php echo $cat['product_count']; ?> items</span>
                                    <a href="category_edit.php?id=<?php echo $cat['category_id']; ?>" class="edit-link">
            ...
            <div class="edit-category">
                <h2 class="section-header">Edit Category Details</h2>
                ...
                <form id="categoryForm" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="categoryDescription" name="description" 
                                  placeholder="Enter category description" rows="3"><?php echo htmlspecialchars($category['description']); ?></textarea>
                    </div>
                        ...
                        <div class="items-container">
                            <div class="items-grid">
                                ...
                                <div class="items-list">
                                        ...
                                        <div class="item-row">
                                            <div class="item-name item-name-col">
                                                <input type="checkbox" id="item<?php echo $product['product_id']; ?>" 
                                                       name="items[]" value="<?php echo $product['product_id']; ?>" checked>
                                    ...
                                    <?php foreach ($uncategorizedProducts as $product): ?>
                                        <div class="item-row">
                                            <div class="item-name item-name-col">
                                                <input type="checkbox" id="item<?php echo $product['product_id']; ?>" 
                    ...
                    <div class="btn-container">
                        <button type="button" class="btn-delete" id="deleteCategory">
    ...
    // Modal functionality for footer/menu items
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function() {
                ...
                items.forEach(item => {
                    ...
                    if ((itemName && itemName.includes(searchTerm)) || (itemSku && itemSku.includes(searchTerm))) {
        ...
        if (deleteCategoryBtn) {
            deleteCategoryBtn.addEventListener('click', function() {
                if (confirm('Are you sure you want to delete this category? Products in this category will become uncategorized.')) {
                    window.location.href = `delete_category.php?id=<?php echo $category_id; ?>`;
                }
            });
        }
    });
